...
{
    clear();
    m_pDocModel = m_pCompiler->compile_file(filename);
//    m_pTree = m_pCompiler->get_final_tree();
...
{
    clear();
    m_pDocModel = m_pCompiler->compile_string(source);
//    m_pTree = m_pCompiler->get_final_tree();
    return m_pCompiler->get_num_errors();
}
...
{
    clear();
    m_pDocModel = m_pCompiler->create_empty();
//    m_pTree = m_pCompiler->get_final_tree();
}
...
{
    clear();
    m_pDocModel = m_pCompiler->create_with_empty_score();
//    m_pTree = m_pCompiler->get_final_tree();
}
...
std::string Document::to_string()
{
    ...
    return exporter.get_source(m_pDocModel);
}


//ImDocument* Document::get_root()
//{
//    return dynamic_cast<ImDocument*>( m_pDocModel->get_root() );
//}
...
{
    DocIterator it(this);
    while (*it != NULL && !(*it)->is_score())
        ++it;
    return dynamic_cast<ImoScore*>( *it );
}
