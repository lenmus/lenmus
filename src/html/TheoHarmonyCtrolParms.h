...
#ifndef __LM_THEOHARMONYCTROLPARMS_H__        //to avoid nested includes
#define __LM_THEOHARMONYCTROLPARMS_H__
...
class lmTheoHarmonyCtrolParms : public lmExerciseParams
{
public:
    lmTheoHarmonyCtrolParms(const wxHtmlTag& tag, int nWidth, int nHeight,
                              int nPercent, long nStyle);
    ~lmTheoHarmonyCtrolParms();
...
protected:
    ...
    DECLARE_NO_COPY_CLASS(lmTheoHarmonyCtrolParms)
};



lmTheoHarmonyCtrolParms::lmTheoHarmonyCtrolParms(const wxHtmlTag& tag, int nWidth, int nHeight,
                                   int nPercent, long nStyle)
    : lmExerciseParams(tag, nWidth, nHeight, nPercent)
{
    ...
    m_pConstrains = new lmHarmonyConstrains(_T("TheoHarmony"));
...
lmTheoHarmonyCtrolParms::~lmTheoHarmonyCtrolParms()
{
...
void lmTheoHarmonyCtrolParms::AddParam(const wxHtmlTag& tag)
{
...
void lmTheoHarmonyCtrolParms::CreateHtmlCell(wxHtmlWinParser *pHtmlParser)
{
    ...
    if (m_sParamErrors != _T("")) {
        ...
        pWnd = new lmTheoHarmonyCtrol((wxWindow*)pHtmlParser->GetWindowInterface()->GetHTMLWindow(), -1,
...
wxString lmTheoHarmonyCtrolParms::ParseAnswerButtons(wxString sParamValue, wxString sFullParam,
                                                 bool* pfValidButtons)
{
    //    cadence_buttons   list of answer buttons to display:
    //                 'terminal, transient, perfect, plagal, imperfect, deceptive, half'
...
wxString lmTheoHarmonyCtrolParms::ParseCadences(wxString sParamValue, wxString sFullParam,
...
lmECadenceType lmTheoHarmonyCtrolParms::CadenceNameToType(wxString sCadence)
{
...
#endif  // __LM_THEOHARMONYCTROLPARMS_H__
