...
#pragma implementation "SystemOldFormat.h"
#endif
...
//=====================================================================================
//lmCtiticalOldLine definition: class to implement the critical line
...
{
public:
    lmCtiticalOldLine(lmColumnOldFormat* pColFmt);
    ~lmCtiticalOldLine();

    void AddNonTimed(lmLineOldFormat* pLineFmt, float rTime);
    void AddEntry(lmLinOldEntry* pEntry);
    ...
    inline lmLinOldEntry* GetLastEntry() { return m_pLineTable->GetLastEntry(); }

protected:
    lmLinOldTbl*   m_pLineTable;       //storage for this line
...
//=====================================================================================
//lmOldBreaksTable definition: table to contain possible break points
//=====================================================================================

//an entry of the BreaksTable
typedef struct lmOldBreaksTimeEntry_Struct
{
    float       rTimepos;
    float       rPriority;
    lmLUnits    uxStart;
...
public:
    ...
    void AddEntry(lmOldBreaksTimeEntry* pBTE);
    void SetPriority();
    ...
    lmOldBreaksTimeEntry* GetFirst();
    lmOldBreaksTimeEntry* GetNext();


private:

    std::list<lmOldBreaksTimeEntry*>               m_BreaksTable;      //the table
    std::list<lmOldBreaksTimeEntry*>::iterator     m_it;               //for GetFirst(), GetNext();
...
//=====================================================================================
//lmOldBreaksTable implementation
//=====================================================================================


lmOldBreaksTable::lmOldBreaksTable()
{
}

lmOldBreaksTable::~lmOldBreaksTable()
{
    std::list<lmOldBreaksTimeEntry*>::iterator it;
    for (it = m_BreaksTable.begin(); it != m_BreaksTable.end(); ++it)
        delete *it;
    m_BreaksTable.clear();
}

void lmOldBreaksTable::AddEntry(float rTime, lmLUnits uxStart, lmLUnits uWidth, bool fInBeam,
                             lmLUnits uxBeam, float rPriority)
{
    lmOldBreaksTimeEntry* pBTE = new lmOldBreaksTimeEntry;
...
void lmOldBreaksTable::AddEntry(lmOldBreaksTimeEntry* pBTE)
{
    AddEntry(pBTE->rTimepos, pBTE->uxStart, pBTE->uxEnd - pBTE->uxStart, pBTE->fInBeam,
             pBTE->uxBeam, pBTE->rPriority);
}

void lmOldBreaksTable::ChangePriority(int iEntry, float rMultiplier)
{
}

wxString lmOldBreaksTable::Dump()
{
...
lmOldBreaksTimeEntry* lmOldBreaksTable::GetFirst()
{
    m_it = m_BreaksTable.begin();
    if (m_it == m_BreaksTable.end())
        return (lmOldBreaksTimeEntry*)NULL;

    return *m_it;
}

lmOldBreaksTimeEntry* lmOldBreaksTable::GetNext()
{
    //advance to next one
    ++m_it;
    if (m_it != m_BreaksTable.end())
...
//=====================================================================================
//lmLinOldEntry implementation
//=====================================================================================

lmLinOldEntry::lmLinOldEntry(lmEOldEnType nType, lmStaffObj* pSO, lmShape* pShape,
                         bool fProlog)
    : m_nType(nType)
    , m_pSO(pSO)
    , m_pShape(pShape)
...
lmLinOldEntry::lmLinOldEntry(lmLinOldEntry* pEntry)
...
void lmLinOldEntry::AssignSpace(lmColumnOldFormat* pTT, float rFactor)
{
...
void lmLinOldEntry::SetNoteRestSpace(lmColumnOldFormat* pTT, float rFactor)
{
    ...
    if (pTT->SpacingMethod() == esm_PropConstantFixed)
    {
	    ...
	    //wxLogMessage(_T("[lmLinOldEntry::SetNoteRestSpace] duration=%s, rel.space=%.2f"),
...
void lmLinOldEntry::Reposition(lmLUnits uxPos)
{
	//reposition Shape
...
wxString lmLinOldEntry::DumpHeader()
{
    //         ...+  ..+   ...+ ..+   +  ..........+........+........+........+........+........+........+........+........+......+
    return _T("Item    Type      ID Prolog   ColumnFormatter    xInit  xAnchor    xLeft     Size    Space   xFinal  SpFixed   SpVar ShpIdx\n");
}

wxString lmLinOldEntry::Dump(int iEntry)
{
...
lmColuOldSplitter::lmColuOldSplitter(lmLinOldTbl* pLineTable)
    : m_pLineTable(pLineTable)
{
}

lmColuOldSplitter::~lmColuOldSplitter()
{
}

void lmColuOldSplitter::ComputeBreakPoints(lmOldBreaksTable* pBT)
{
    ...
    const lmLineEntryOldItera itEnd = m_pLineTable->End();
	lmLineEntryOldItera it = m_pLineTable->Begin();
...
lmLineOldFormat::lmLineOldFormat(lmLinOldTbl* pTable)
    : m_pTable(pTable)
    , m_itCur(pTable->End())
    , m_rCurTime(0.0f)
	, m_uxCurPos(0.0f)
...
lmLineOldFormat::~lmLineOldFormat()
{
}

void lmLineOldFormat::StartTraversing(lmLUnits uSpaceAfterProlog)
{
    m_rCurTime = 0.0f;
...
void lmLineOldFormat::FindFirstTimed(lmLUnits uSpaceAfterProlog)
{
    ...
    if (m_itCur != m_pTable->End())
    {
        ...
        lmLineEntryOldItera itEnd = m_pTable->End();
...
lmLinOldEntry* lmLineOldFormat::GetMinAlignedObjForTime(float rTime)
{
    ...
    lmLinOldEntry* pEntry = (lmLinOldEntry*)NULL;
	lmLineEntryOldItera itEnd = m_pTable->End();
	for (lmLineEntryOldItera it = m_itCur; it != itEnd && IsEqualTime((*it)->m_rTimePos, rTime); ++it)
    {
...
float lmLineOldFormat::AssignPositionToObjectsAtTime(float rTime,
                                    ...
                                    lmLUnits* pMaxPos)
{
    ...
    *pMaxPos = 0.0f;
    const lmLineEntryOldItera itEnd = m_pTable->End();
	lmLineEntryOldItera itLastNotTimed = itEnd;
	while (m_itCur != itEnd &&
	...
	if (m_itCur != m_pTable->End())
	{
		...
		if (itLastNotTimed != m_pTable->End())
		{
            ...
            lmLineEntryOldItera itStart = m_itCur;
            lmLineEntryOldItera it = m_itCur;
            it--;
    ...
    else
    {
		...
		if (itLastNotTimed != m_pTable->End() &&
            (*itLastNotTimed)->m_nType == lm_eOldOmega &&
...
void lmLineOldFormat::FromCurrentPosAddNonTimedToLine(lmLinOldTbl* pLineTable)
{
	lmLineEntryOldItera it = m_itFirstObj;
	lmLineEntryOldItera itEnd = m_pTable->End();
...
lmLUnits lmLineOldFormat::GetSpaceNonTimedForTime(float rTime)
{
    ...
    {
        lmLineEntryOldItera it = m_itFirstTimed;      //to point to last non-timed object
        --it;
        return (*it)->m_xFinal - (*m_itFirstObj)->m_xLeft;
    }
}

lmLUnits lmLineOldFormat::GetAnchorForTime(float rTime)
{
...
lmLUnits lmLineOldFormat::IntitializeSpacingAlgorithm(lmLUnits uSpaceAfterProlog)
{
...
void lmLineOldFormat::AssignSpace(lmColumnOldFormat* pColFmt, float rFactor)
{
    ...
    for (lmLineEntryOldItera it = m_pTable->Begin(); it != m_pTable->End(); ++it)
	{
        (*it)->AssignSpace(pColFmt, rFactor);
    }
}

void lmLineOldFormat::AddLongestNonTimedObjectsToCriticalLine(
                                    lmCtiticalOldLine* pCriticalLine,
                                    float rTime)
{
...
lmLinOldTbl::lmLinOldTbl(int nInstr, int nVoice)
    : m_nInstr(nInstr)
	, m_nVoice(nVoice)
{
}

lmLinOldTbl::~lmLinOldTbl()
{
    for (lmLineEntryOldItera it = m_LineEntries.begin(); it != m_LineEntries.end(); ++it)
		delete *it;

    m_LineEntries.clear();
}

lmLinOldEntry* lmLinOldTbl::AddEntry(lmEOldEnType nType, lmStaffObj* pSO,
                                          lmShape* pShape, bool fProlog)
{
    return NewEntry(nType, pSO, pShape, fProlog);
}

lmLinOldEntry* lmLinOldTbl::NewEntry(lmEOldEnType nType, lmStaffObj* pSO,
                                          lmShape* pShape, bool fProlog, lmLUnits uSpace)
{
    lmLinOldEntry* pEntry = new lmLinOldEntry(nType, pSO, pShape, fProlog);
    PushBack(pEntry);
    if (nType == lm_eOldAlpha)
        pEntry->m_uFixedSpace = uSpace;
	return pEntry;
}

wxString lmLinOldTbl::DumpMainTable()
{
    wxString sMsg = wxString::Format(_T("TimeLine table dump. Instr=%d, voice=%d \n"),
									 GetInstrument(), GetVoice() );
    sMsg += _T("===================================================================\n\n");
...
void lmLinOldTbl::ClearDirtyFlags()
{
...
lmLUnits lmLinOldTbl::GetLineWidth()
{
	//Return the size of the measure represented by this line or 0 if
	//no omega entry.

	if (m_LineEntries.size() > 0 && m_LineEntries.back()->m_nType == lm_eOldOmega)
...
lmLUnits lmLinOldTbl::GetMaxXFinal()
{
...
void lmLinOldTbl::CreateAlphaEntry(lmLUnits uxStart, lmLUnits uSpace)
{
    NewEntry(lm_eOldAlpha, (lmStaffObj*)NULL, (lmShape*)NULL, false, uSpace);
...
lmCtiticalOldLine::lmCtiticalOldLine(lmColumnOldFormat* pColFmt)
{
    m_pLineTable = new lmLinOldTbl(0, 0);
}

lmCtiticalOldLine::~lmCtiticalOldLine()
{
...
void lmCtiticalOldLine::AddNonTimed(lmLineOldFormat* pLineFmt, float rTime)
{
...
void lmCtiticalOldLine::AddEntry(lmLinOldEntry* pEntry)
{
...
lmLUnits lmCtiticalOldLine::RedistributeSpace(lmLUnits uNewBarSize, lmLUnits uNewStart,
                                           lmLUnits uOldBarSize, lmBoxSlice* pBSlice)
{
    //Creates the Pos<->Time table (m_PosTimes)

#if lmDUMP_TABLES
    wxLogMessage(_T("[lmCtiticalOldLine::RedistributeSpace] Before space redistribution. uNewBarSize=%.2f, uNewStart=%.2f, uOldBarSize=%.2f"),
	...
	{
        lmLinOldEntry* pTPE = *it;
        if (pTPE->m_nType == lm_eOldStaffObj)
        {
        ...
        else if (pTPE->m_nType == lm_eOldOmega)
        {
            ...
            if (pTPE->m_pSO)
            {
                ...
                m_PosTimes.push_back(tPosTime);
				//wxLogMessage(_T("[lmCtiticalOldLine::RedistributeSpace] Reposition bar: uBarPosition=%.2f, uShiftBar=%.2f, xLeft=%.2f, uxAnchor=%.2f"),
...
void lmCtiticalOldLine::InitializeToGetTimepos()
{
    m_rLastTimepos = -1.0f;
    m_it = m_PosTimes.begin();
}

lmLUnits lmCtiticalOldLine::GetPosForStaffobj(lmStaffObj* pSO, float rTime)
{
...
lmLUnits lmCtiticalOldLine::GetTimepos(float rTime)
{
    ...
    //Error
    #ifdef __WXDEBUG__
        g_pLogger->LogForensic(_T("[lmCtiticalOldLine::GetTimepos] Looking for time %.2f"), rTime);
        if (it != m_PosTimes.end())
            g_pLogger->LogForensic(_T("[lmCtiticalOldLine::GetTimepos] m_it is pointing to rTime %.2f"), (*it).rTimepos);
        else
            g_pLogger->LogForensic(_T("[lmCtiticalOldLine::GetTimepos] m_it is pointing to end"));
...
wxString lmCtiticalOldLine::DumpPosTimes()
{
...
lmColumnOldFormat::lmColumnOldFormat(lmColuOldStorage* pStorage, float rSpacingFactor,
    ...
    , m_pCriticalLine((lmCtiticalOldLine*)NULL)
    , m_pColStorage(pStorage)
{
}

lmColumnOldFormat::~lmColumnOldFormat()
{
    Initialize();
}

lmLUnits lmColumnOldFormat::TenthsToLogical(lmTenths rTenths, int nStaff)
{
    return m_pColStorage->TenthsToLogical(rTenths, nStaff);
}

void lmColumnOldFormat::Initialize()
{
...
lmBarline* lmColumnOldFormat::GetBarline()
{
    ...
    lmLinOldEntry* pEntry = m_pCriticalLine->GetLastEntry();
    wxASSERT(pEntry->m_nType == lm_eOldOmega);
    return (lmBarline*)pEntry->m_pSO;
}

void lmColumnOldFormat::DoSpacing(bool fTrace)
{
...
lmLUnits lmColumnOldFormat::ComputeSpacing(float rFactor)
{
    //Spacing algorithm. Builds the critical line that will constrain the minimum space
    //required for this column.
    //Returns the resulting minimum column width

	//Assigne spacing and positions to each object in each line, independently of 
    //any other line
    ...
    m_fCreateCriticalLine = (m_pCriticalLine == (lmCtiticalOldLine*)NULL );
    if (!m_fCreateCriticalLine)
        wxLogMessage(_T("[lmColumnOldFormat::ComputeSpacing] Critical line exists!"));

	lmLUnits uxCurPos = 0.0f;
    m_pLongestNonTimedLineFmt = (lmLineOldFormat*)NULL;

    lmLinesOldItera itEnd = m_pColStorage->End();
    for (lmLinesOldItera it=m_pColStorage->Begin(); it != itEnd; ++it)
	{
        //create formatters for the lines
        lmLineOldFormat* pLineFmt = new lmLineOldFormat(*it);
    ...
    //Now we create the critical line (if requested) and enter a loop to process timed
	...
	while(fContinue)
    {
        ...
        m_pShortestEntry = (lmLinOldEntry*)NULL;
        m_rShortestObjectDuration = 100000000.0f;       //any too big value
		for (lmLineFormattersOldItera it = m_LineFormatters.begin(); it != m_LineFormatters.end(); ++it)
		{
            ...
            //If computing critical line, find the aligned object at current timepos
            //with minimum time duration. If several objects with this minimum duration,
            //take the widest one. Found values are left  At each loop cycle we retain the object with minimal
            //duration in m_rShortestObjectDuration and m_pShortestEntry.
            if (m_fCreateCriticalLine)
            {
                lmLinOldEntry* pEntry = (*it)->GetMinAlignedObjForTime(m_rCurTime);
                SelectTimedObjIfItHasMinimalDuration(pEntry);
            }
		}
        ...
        if (m_fCreateCriticalLine)
        {
            m_pLongestNonTimedLineFmt->
                AddLongestNonTimedObjectsToCriticalLine(m_pCriticalLine, m_rCurTime);
            AddShorterTimedObjectToCriticalLine(m_pShortestEntry);
        }
        ...
        //take more space. Result is left in m_pLongestNonTimedLineFmt.
        if (m_fCreateCriticalLine)
            DetermineLineWithLongestNonTimedObjects();
    ...
    lmLinOldEntry* pOmegaEntry = (lmLinOldEntry*)NULL;
    lmLUnits uOmegaSize = -100000000.0f;        //any too low value
    float rOmegaTime = 0.0f;
    int iLine=0;
	for (lmLinesOldItera it=m_pColStorage->Begin(); it != m_pColStorage->End(); ++it)
	{
        ...
        if (m_fCreateCriticalLine)
        {
            lmLinOldEntry* pOmega = (*it)->GetLastEntry();
            if (pOmega->m_nType == lm_eOldOmega)
            {
                if (uOmegaSize < pOmega->GetTotalSize()
                    || IsLowerTime(rOmegaTime, pOmega->m_rTimePos) )
                {
                    uOmegaSize = pOmega->GetTotalSize();
                    rOmegaTime = pOmega->m_rTimePos;
                    pOmegaEntry = pOmega;
                }
            }
        }
    ...
    //if computing critical line, add the omega entry
    if (m_fCreateCriticalLine && pOmegaEntry)
    {
    ...
    //if (m_fCreateCriticalLine)
    //{
    //    wxLogMessage(_T("[lmColumnOldFormat::ComputeSpacing] Critical line:"));
    //    wxLogMessage(_T("*******************************************************************\n"));
    //    wxLogMessage(m_pCriticalLine->DumpMainTable());
    //}

    //wxLogMessage(_T("[lmColumnOldFormat::ComputeSpacing] measure size = %.2f"), uMeasureSize);


    //delete line formatters
    std::vector<lmLineOldFormat*>::iterator itL;
...
lmLUnits lmColumnOldFormat::RedistributeSpace(lmLUnits uNewStart, lmBoxSlice* pBSlice)
{
	...
	for (lmLinesOldItera it=m_pColStorage->Begin(); it != m_pColStorage->End(); ++it)
	{
        lmLinOldResizer oResizer(*it);
		oResizer.RepositionShapes(m_pCriticalLine, uNewBarSize, uNewStart, uOldBarSize);
        oResizer.InformAttachedObjs();
    }

    //wxLogMessage(_T("[lmColumnOldFormat::RedistributeSpace] uNewBarSize=%.2f, uNewStart=%.2f, uOldBarSize=%.2f"),
    //             uNewBarSize, uNewStart, uOldBarSize );

#if lmDUMP_TABLES
    wxLogMessage(_T("[lmColumnOldFormat::RedistributeSpace] After repositioning shapes"));
    wxLogMessage( DumpColumnData() );
#endif
...
bool lmColumnOldFormat::GetOptimumBreakPoint(lmLUnits uAvailable, float* prTime,
                                          lmLUnits* puWidth)
{
    ...
    lmOldBreaksTable* pBreaks = ComputeBreaksTable();

    //wxLogMessage( DumpColumnData() );

    //select highest entry with space <= uAvailable
    lmOldBreaksTimeEntry* pBTE = pBreaks->GetFirst();
    lmOldBreaksTimeEntry* pSel = (lmOldBreaksTimeEntry*)NULL;
    ...
    //wxLogMessage(_T("[lmColumnOldFormat::GetOptimumBreakPoint] uAvailable=%.2f, returned=%.2f, time=%.2f"),
...
lmOldBreaksTable* lmColumnOldFormat::ComputeBreaksTable()
{
    ...
    std::vector<lmOldBreaksTable*> partialTables;
	for (lmLinesOldItera itTL = m_pColStorage->Begin(); itTL != m_pColStorage->End(); ++itTL)
	{
        lmOldBreaksTable* pBT = new lmOldBreaksTable();
        lmColuOldSplitter oSplitter(*itTL);
    ...
    lmOldBreaksTable* pTotalBT = new lmOldBreaksTable();
    std::vector<lmOldBreaksTable*>::iterator itBT;
    for (itBT = partialTables.begin(); itBT != partialTables.end(); ++itBT)
    {
        if (pTotalBT->IsEmpty())
        {
            //just copy entries
            lmOldBreaksTimeEntry* pEP = (*itBT)->GetFirst();       //pEP Entry from Partial list
            while (pEP)
            {
...
void lmColumnOldFormat::InitializeCriticalLine()
{
    //create the line object
    m_pCriticalLine = new lmCtiticalOldLine(this);
    ...
    m_pCriticalLine->AddEntry( m_pColStorage->Front()->GetAlphaEntry() );
        ...
        wxLogMessage(_T("[lmColumnOldFormat::ComputeSpacing] Start creating Critical Line. Added alpha entry"));
        wxLogMessage( m_pCriticalLine->DumpMainTable() );
    #endif
}

void lmColumnOldFormat::AddShorterTimedObjectToCriticalLine(lmLinOldEntry* pShortestEntry)
{
...
bool lmColumnOldFormat::AlignObjectsAtCurrentTimeAndGetNextTime(lmLUnits uxPos,
                                                                float rFactor)
{
	//Process all timed objects placed at time m_rCurTime to reposition them and determine
    //next timepos (m_rNextTime) and its start position (m_uxNextTimeStartPosition).
...
void lmColumnOldFormat::DetermineLineWithLongestNonTimedObjects()
{
...
void lmColumnOldFormat::SelectTimedObjIfItHasMinimalDuration(lmLinOldEntry* pEntry)
{
...
//-------------------------------------------------------------------------------------
// lmSystemOldFormat implementation
//-------------------------------------------------------------------------------------

lmSystemOldFormat::lmSystemOldFormat(float rSpacingFactor, lmESpacingMethod nSpacingMethod,
...
void lmSystemOldFormat::StarBarMeasurements(int iCol, int nInstr, lmLUnits uxStart,
                                            lmVStaff* pVStaff, lmLUnits uSpace)
{
    ...
    if (m_ColFormatters.size() == (size_t)iCol)
    {
        ...
        lmColuOldStorage* pStorage = new lmColuOldStorage();
        m_ColStorage.push_back(pStorage);

        //create a lines builder object for this column
        pLB = new lmLinOldBuilder(pStorage);
        m_LinesBuilder.push_back(pLB);

        //create the column formatter object
        lmColumnOldFormat* pColFmt = new lmColumnOldFormat(pStorage, m_rSpacingFactor,
...
void lmSystemOldFormat::IncludeObject(int iCol, int nInstr, lmStaffObj* pSO,
...
void lmSystemOldFormat::IncludeBarlineAndTerminateBarMeasurements(int iCol, lmStaffObj* pSO, 
...
void lmSystemOldFormat::TerminateBarMeasurementsWithoutBarline(int iCol, lmLUnits xStart)
{
...
void lmSystemOldFormat::DiscardMeasurementsForColumn(int iCol)
{
...
void lmSystemOldFormat::DoColumnSpacing(int iCol, bool fTrace)
{
    m_ColFormatters[iCol]->DoSpacing(fTrace);
}

lmLUnits lmSystemOldFormat::RedistributeSpace(int iCol, lmLUnits uNewStart,
                                              lmBoxSlice* pBSlice)
{
    return m_ColFormatters[iCol]->RedistributeSpace(uNewStart, pBSlice);
}

void lmSystemOldFormat::IncrementColumnSize(int iCol, lmLUnits uIncr)
{
    m_ColFormatters[iCol]->IncrementColumnSize(uIncr);
}

lmLUnits lmSystemOldFormat::GetStartPositionForColumn(int iCol)
{
    return m_ColStorage[iCol]->GetStartOfBarPosition();
}

lmLUnits lmSystemOldFormat::GetMinimumSize(int iCol)
{
    return m_ColFormatters[iCol]->GetMinimumSize();
}

bool lmSystemOldFormat::GetOptimumBreakPoint(int iCol, lmLUnits uAvailable,
                                        float* prTime, lmLUnits* puWidth)
{
    return m_ColFormatters[iCol]->GetOptimumBreakPoint(uAvailable, prTime, puWidth);
}

lmBarline* lmSystemOldFormat::GetColumnBarline(int iCol)
{
    return m_ColFormatters[iCol]->GetBarline();
}

void lmSystemOldFormat::ClearDirtyFlags(int iCol)
{
    m_ColStorage[iCol]->ClearDirtyFlags();
}

wxString lmSystemOldFormat::DumpColumnData(int iCol)
{
...
int lmSystemOldFormat::GetNumObjectsInColumnLine(int iCol, int iLine)
{
    //iCol, iLine = [0..n-1]
...
//-------------------------------------------------------------------------------------
// lmColuOldStorage implementation: encapsulates the table of lines for a column
//-------------------------------------------------------------------------------------

lmColuOldStorage::lmColuOldStorage()
{
}
...
void lmColuOldStorage::DeleteLines()
{
	for (lmLinesOldItera it=m_Lines.begin(); it != m_Lines.end(); ++it)
	{
		delete *it;
	}
	m_Lines.clear();
...
lmLUnits lmColuOldStorage::TenthsToLogical(lmTenths rTenths, int nStaff)
{
    wxASSERT(nStaff > 0);
	return m_pStaff[nStaff-1]->TenthsToLogical(rTenths);
}

lmLinesOldItera lmColuOldStorage::FindLineForInstrAndVoice(int nInstr, int nVoice)
{
...
lmLinOldTbl* lmColuOldStorage::OpenNewLine(int nInstr, int nVoice, lmLUnits uxStart,
                                               lmLUnits uSpace)
{
    lmLinOldTbl* pLineTable = new lmLinOldTbl(nInstr, nVoice);
    m_Lines.push_back(pLineTable);
    pLineTable->CreateAlphaEntry(uxStart, uSpace);

    return pLineTable;
}

wxString lmColuOldStorage::DumpColumnStorage()
{
...
void lmColuOldStorage::ClearDirtyFlags()
{
    //Clear flag 'Dirty' in all StaffObjs of this table. This has nothing to do with 
    //lmColuOldStorage purposes, but its is a convenient place to write a method
    //for doing this.
...
lmLUnits lmColuOldStorage::GetGrossBarSize()
{
...
lmLUnits lmColuOldStorage::GetStartOfBarPosition()
{
    //returns the x position for the start of the bar column

    //the bar starts at the xLeft of the alfa entry
    lmLinOldEntry* pEntry = m_Lines.front()->GetAlphaEntry();
    wxASSERT(pEntry->m_nType == lm_eOldAlpha);
...
//-------------------------------------------------------------------------------------
// lmLinOldBuilder implementation: encapsulates the algorithms to split a column
// into lines and to store them in the received column storage
//-------------------------------------------------------------------------------------

lmLinOldBuilder::lmLinOldBuilder(lmColuOldStorage* pStorage)
    : m_pColStorage(pStorage)
	, m_pCurEntry((lmLinOldEntry*)NULL)
{
    Initialize();
}

lmLinOldBuilder::~lmLinOldBuilder()
{
}

void lmLinOldBuilder::Initialize()
{
    ResetDefaultStaffVoices();
}

void lmLinOldBuilder::ResetDefaultStaffVoices()
{
    for(int i=0; i < lmMAX_STAFF; i++)
        m_nStaffVoice[i] = 0;
}

void lmLinOldBuilder::StarMeasurementsForInstrument(int nInstr, lmLUnits uxStart,
                                                      lmVStaff* pVStaff, lmLUnits uSpace)
{
    CreateLinesForEachStaff(nInstr, uxStart, pVStaff, uSpace);
}

void lmLinOldBuilder::CreateLinesForEachStaff(int nInstr, lmLUnits uxStart,
                                                lmVStaff* pVStaff, lmLUnits uSpace)
{
...
void lmLinOldBuilder::StartLine(int nInstr, int nVoice, lmLUnits uxStart, lmLUnits uSpace)
{
    ...
    lmLinOldTbl* pLineTable = m_pColStorage->OpenNewLine(nInstr, nVoice, uxStart, uSpace);

    //set pointer to last added entry in created line
	m_pCurEntry = pLineTable->GetLastEntry();

    //created line is set as 'current line' to receive new data. 
    m_itCurLine = m_pColStorage->GetLastLine();

	//wxLogMessage(_T("[lmLinOldBuilder::StartLine] New line started for instr=%d, voice=%d"),
 //       nInstr, nVoice);
}

void lmLinOldBuilder::CloseLine(lmStaffObj* pSO, lmShape* pShape, lmLUnits xStart)
{
	//close current line.

    m_pCurEntry = (*m_itCurLine)->AddEntry(lm_eOldOmega, pSO, pShape, false);
    m_pCurEntry->m_xLeft = xStart;
    m_pCurEntry->m_xInitialLeft = xStart;
}

void lmLinOldBuilder::IncludeObject(int nInstr, lmStaffObj* pSO, lmShape* pShape,
                                   bool fProlog, int nStaff)
{
	...
	m_pCurEntry = (*m_itCurLine)->AddEntry(lm_eOldStaffObj, pSO, pShape, fProlog);
...
int lmLinOldBuilder::DecideVoiceToUse(lmStaffObj* pSO, int nStaff)
{
...
void lmLinOldBuilder::EndOfData()
{
...
//-------------------------------------------------------------------------------------
// lmLinOldResizer implementation:
...
lmLinOldResizer::lmLinOldResizer(lmLinOldTbl* pLineTable)
    : m_pLineTable(pLineTable)
{
}

lmLinOldResizer::~lmLinOldResizer()
{
}

lmLUnits lmLinOldResizer::RepositionShapes(lmCtiticalOldLine* pCriticalLine,
                                           ...
                                           lmLUnits uOldBarSize)
{
    ...
    lmLinOldEntry* pTPE = m_pLineTable->Front();
	...
	//wxLogMessage(_T("[lmLinOldResizer::RepositionShapes] Reposition: uNewBarSize=%.2f  uNewStart=%.2f  uOldBarSize=%.2f"),
	//			 uNewBarSize, uNewStart, uOldBarSize );
    ...
    for (; it != m_pLineTable->End(); ++it)
	{
        ...
        else if (pTPE->m_rTimePos < 0.0f && pTPE->m_nType == lm_eOldStaffObj)
        {
        ...
        else if (pTPE->m_nType == lm_eOldOmega)
        {
            ...
            if (pTPE->m_pSO)
            {
                ...
                //wxLogMessage(_T("[lmLinOldResizer::RepositionShapes] Reposition bar: uBarPosition=%.2f, uShiftBar=%.2f"),
...
void lmLinOldResizer::InformAttachedObjs()
{
    ...
    for (lmLineEntryOldItera it = m_pLineTable->Begin(); it != m_pLineTable->End(); ++it)
	{
        if ((*it)->m_nType == lm_eOldStaffObj)
        {
            if ((*it)->m_pSO->IsNote()
