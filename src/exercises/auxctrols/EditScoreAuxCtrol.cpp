...
#pragma implementation "EditScoreAuxCtrol.h"
#endif
...
BEGIN_EVENT_TABLE(lmEditScoreAuxCtrol, wxWindow)
    EVT_PAINT(lmEditScoreAuxCtrol::OnPaint)
    EVT_SIZE(lmEditScoreAuxCtrol::OnSize)
    LM_EVT_SCORE_HIGHLIGHT(lmEditScoreAuxCtrol::OnVisualHighlight)
END_EVENT_TABLE()


lmEditScoreAuxCtrol::lmEditScoreAuxCtrol(wxWindow* parent, wxWindowID id, lmScore* pScore,
                                 const wxPoint& pos,
...
void lmEditScoreAuxCtrol::ComputeScale()
{
    ...
    //wxLogMessage(_T("[lmEditScoreAuxCtrol::ComputeScale] Char height=%d px, Staff height=%d px, uTextHeight=%.2f LU, m_rScale=%f"),
    //             GetCharHeight(), nStaffHeight, uTextHeight, m_rScale);

    SetScale(m_rScale);
}

lmEditScoreAuxCtrol::~lmEditScoreAuxCtrol()
{
    DoStopSounds();     //stop any possible score being played
}

void lmEditScoreAuxCtrol::SetMargins(lmLUnits nLeft, lmLUnits nRight, lmLUnits nTop)
{
    m_nTopMargin = nTop;
    m_nLeftMargin = nLeft;
...
void lmEditScoreAuxCtrol::SetScale(float rScale)
{
    //rScale is the zooming factor
    m_rZoom = rScale;
    m_rScale = rScale * lmSCALE;

    //wxLogMessage(_T("[lmEditScoreAuxCtrol::SetScale]rScale=%f, lmSCALE=%f, m_rScale=%f"), rScale, lmSCALE, m_rScale);
    ResizePaper();
}

void lmEditScoreAuxCtrol::ResizePaper()
{
    ...
    if (g_pLogger->IsAllowedTraceMask(_T("lmEditScoreAuxCtrol"))) GetPixelsPerLU();
    g_pLogger->LogTrace(_T("lmEditScoreAuxCtrol"),
        _T("[lmEditScoreAuxCtrol::ResizePaper] :\n")
...
double lmEditScoreAuxCtrol::GetPixelsPerLU()
{
    wxClientDC dc(this);
    dc.SetMapMode(lmDC_MODE);
    dc.SetUserScale( m_rScale, m_rScale );
    ...
    wxLogMessage(_T("[lmEditScoreAuxCtrol::GetPixelsPerLU] m_rScale=%f, DisplayPixelsPerLU=(%f, %f)"),
...
void lmEditScoreAuxCtrol::SetBaseScale()
{
    ...
    wxLogMessage(_T("[lmEditScoreAuxCtrol::SetBaseScale] : \n")
...
void lmEditScoreAuxCtrol::OnSize(wxSizeEvent& WXUNUSED(event))
{
    ...
    //    _T("[lmEditScoreAuxCtrol::OnSize]New window size = (%d, %d)"), xPixels, yPixels));
    ////END DBG -----------------------------------------------------------------
...
void lmEditScoreAuxCtrol::OnPaint(wxPaintEvent &WXUNUSED(event))
{
    bool fClear = true;
    ...
    if (m_pScore && !m_fHidden)
    {
        ...
        //wxLogMessage(wxString::Format(
        //    _T("[lmEditScoreAuxCtrol::OnPaint]dxyBitmap = (%d, %d)"), dxBitmap, dyBitmap));
...
void lmEditScoreAuxCtrol::DisplayMessage(wxString sMsg, lmLUnits posMsg, bool fClearScore)
{
...
void lmEditScoreAuxCtrol::DisplayScore(lmScore* pScore, bool fClearMessage)
{
    //wxLogMessage(_T("[lmEditScoreAuxCtrol::DisplayScore] ScoreID=%d"), pScore->GetID() );
    SetScore(pScore, false);        //false: not hidden
    if (fClearMessage) m_fDisplayMessage = false;
    Refresh();
}

void lmEditScoreAuxCtrol::SetScore(lmScore* pScore, bool fHidden)
{
    m_fHidden = fHidden;
    m_pScore = pScore;
    if (m_pScore)
...
void lmEditScoreAuxCtrol::HideScore(bool fHide)
{
    m_fHidden = fHide;
    Refresh();
}

void lmEditScoreAuxCtrol::PlayScore(bool fVisualTracking, bool fCountOff,
...
void lmEditScoreAuxCtrol::PlayMeasure(int nMeasure, bool fVisualTracking, lmEPlayMode nPlayMode,
        long nMM)
{
    if (m_pScore) {
...
void lmEditScoreAuxCtrol::Pause()
{
    if (m_pScore) {
        m_pScore->Pause();
    }
...
void lmEditScoreAuxCtrol::OnVisualHighlight(lmScoreHighlightEvent& event)
{
    if (!m_pScore) return;
    if (m_pScore->GetID() != event.GetScoreID() )
    {
        //the event is not for the score controlled by this control
        wxLogMessage(_T("[lmEditScoreAuxCtrol::OnVisualHighlight] Ignored higlight event: this score ID =%d, target score ID = %d"),
...
void lmEditScoreAuxCtrol::SourceLDP()
{
    if (!m_pScore) return;
...
void lmEditScoreAuxCtrol::SourceXML(int nIndent)
{
    if (!m_pScore) return;
...
void lmEditScoreAuxCtrol::DumpMidiEvents()
{
    if (!m_pScore) return;
...
void lmEditScoreAuxCtrol::DoStopSounds()
{
