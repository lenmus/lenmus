...
BEGIN_EVENT_TABLE(lmDlgCfgIdfyNotes, wxDialog)
    EVT_BUTTON(lmID_ACCEPT, lmDlgCfgIdfyNotes::OnAcceptClicked )
    EVT_BUTTON(lmID_CANCEL, lmDlgCfgIdfyNotes::OnCancelClicked )
    EVT_CHOICE(lmID_KEY_SIGNATURE, lmDlgCfgIdfyNotes::OnDataChanged )
    EVT_RADIOBUTTON(lmID_NOTES_FROM_KEY, lmDlgCfgIdfyNotes::OnRadioFromKeySignature )
    EVT_RADIOBUTTON(lmID_NOTES_FROM_SELECTED, lmDlgCfgIdfyNotes::OnRadioSelectedNotes )
    EVT_CHECKBOX(lmID_CHK_NOTE, lmDlgCfgIdfyNotes::OnDataChanged )
    EVT_RADIOBOX(lmID_CLEF, lmDlgCfgIdfyNotes::OnDataChanged )
    EVT_RADIOBOX(lmID_PLAY, lmDlgCfgIdfyNotes::OnDataChanged )
    EVT_RADIOBOX(lmID_RAD_OCTAVES, lmDlgCfgIdfyNotes::OnDataChanged )
END_EVENT_TABLE()
    ...
    , m_pConstrains(pConstrains)
{
    ...
    for (int i = lmMIN_KEY; i <= lmMAX_KEY; i++)
    {
        ...
        m_pCboKeySignature->Append( lmGetKeySignatureName(nKey) );
    }
    m_pCboKeySignature->SetStringSelection( lmGetKeySignatureName(pConstrains->GetKeySignature()) );
    ...
    {
        case lmE_Sol:   m_pRadClefs->SetSelection(0);   break;
...
lmDlgCfgIdfyNotes::~lmDlgCfgIdfyNotes()
{
}

void lmDlgCfgIdfyNotes::CreateControls()
{
...
void lmDlgCfgIdfyNotes::OnDataChanged(wxCommandEvent& WXUNUSED(event))
{
    VerifyData();
}

void lmDlgCfgIdfyNotes::OnAcceptClicked(wxCommandEvent& WXUNUSED(event))
{
    ...
    if ( m_pRadFromKeySignature->GetValue() )
        m_pConstrains->SetKeySignature( static_cast<lmEKeySignatures>(m_pCboKeySignature->GetSelection()) );
    ...
    {
        case 0:   m_pConstrains->SetClef(lmE_Sol);   break;
...
bool lmDlgCfgIdfyNotes::VerifyData()
{
    ...
    if ( m_pRadFromKeySignature->GetValue() )
    {
        lmEKeySignatures nKey = static_cast<lmEKeySignatures>(m_pCboKeySignature->GetSelection());
        int nAcc[7];
...
void lmDlgCfgIdfyNotes::OnRadioFromKeySignature(wxCommandEvent& WXUNUSED(event))
{
    EnableDisableNotesSelection(false);
}

void lmDlgCfgIdfyNotes::OnRadioSelectedNotes(wxCommandEvent& WXUNUSED(event))
{
    EnableDisableNotesSelection(true);
}

void lmDlgCfgIdfyNotes::EnableDisableNotesSelection(bool fEnable)
{
