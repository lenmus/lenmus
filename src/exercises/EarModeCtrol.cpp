...
//
//-------------------------------------------------------------------------------------
/*! @file EarModeCtrol.cpp
    @brief Implementation file for class lmEarModeCtrol
    @ingroup html_controls
*/
#if defined(__GNUG__) && !defined(NO_GCC_PRAGMA)
#pragma implementation "EarModeCtrol.h"
#endif
...
//------------------------------------------------------------------------------------
// Implementation of lmEarModeCtrol
...
BEGIN_EVENT_TABLE(lmEarModeCtrol, wxWindow)
    EVT_COMMAND_RANGE (ID_BUTTON, ID_BUTTON+2, wxEVT_COMMAND_BUTTON_CLICKED, lmEarModeCtrol::OnRespButton)
    EVT_SIZE            (lmEarModeCtrol::OnSize)

    LM_EVT_URL_CLICK    (ID_LINK_SEE_SOURCE, lmEarModeCtrol::OnDebugShowSourceScore)
    LM_EVT_URL_CLICK    (ID_LINK_DUMP, lmEarModeCtrol::OnDebugDumpScore)
    LM_EVT_URL_CLICK    (ID_LINK_MIDI_EVENTS, lmEarModeCtrol::OnDebugShowMidiEvents)

    LM_EVT_URL_CLICK    (ID_LINK_NEW_PROBLEM, lmEarModeCtrol::OnNewProblem)
    LM_EVT_URL_CLICK    (ID_LINK_PLAY, lmEarModeCtrol::OnPlay)
    LM_EVT_URL_CLICK    (ID_LINK_SOLUTION, lmEarModeCtrol::OnDisplaySolution)
    LM_EVT_URL_CLICK    (ID_LINK_SETTINGS, lmEarModeCtrol::OnSettingsButton)
END_EVENT_TABLE()

IMPLEMENT_CLASS(lmEarModeCtrol, wxWindow)

lmEarModeCtrol::lmEarModeCtrol(wxWindow* parent, wxWindowID id, 
...
void lmEarModeCtrol::EnableButtons(bool fEnable)
{
    for (int i=0; i < 3; i++) {
...
void lmEarModeCtrol::OnSettingsButton(wxCommandEvent& event)
{
...
void lmEarModeCtrol::OnSize(wxSizeEvent& event)
{
    //wxLogMessage(_T("OnSize at lmEarModeCtrol"));
    Layout();

}

void lmEarModeCtrol::OnPlay(wxCommandEvent& event)
{
    Play();
}

void lmEarModeCtrol::OnNewProblem(wxCommandEvent& event)
{
    NewProblem();
}

void lmEarModeCtrol::OnDisplaySolution(wxCommandEvent& event)
{
...
void lmEarModeCtrol::OnRespButton(wxCommandEvent& event)
{
...
void lmEarModeCtrol::NewProblem()
{
    ...
    EKeySignatures nKey = oGenerator.RandomKeySignature();
    
    //generate the chords: I, V, IV, I
...
void lmEarModeCtrol::DisplaySolution()
{
...
void lmEarModeCtrol::OnDebugShowSourceScore(wxCommandEvent& event)
{
    m_pScoreCtrol->SourceLDP();
}

void lmEarModeCtrol::OnDebugDumpScore(wxCommandEvent& event)
{
    m_pScoreCtrol->Dump();
}

void lmEarModeCtrol::OnDebugShowMidiEvents(wxCommandEvent& event)
{
    m_pScoreCtrol->DumpMidiEvents();
}

void lmEarModeCtrol::ResetExercise()
{
    //clear the canvas
    m_pScoreCtrol->DisplayMessage(_T(""), 0, true);     //true: clear the canvas
