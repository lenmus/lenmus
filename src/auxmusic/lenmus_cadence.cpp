...
bool Cadence::Create(ECadenceType nCadenceType, EKeySignature nKey, bool fUseGrandStaff)
{
    ...
    if (nValidFirst == 0) {
        ...
        TraceCadence("[Cadence::Create] Selected : %s, %s, %s, %s",
                    ...
                    aFirstChord[iC].GetPrintName(3).wx_str() );
    }
								  ...
								  Chord* pChord, int nInversion)
{
    ...
    for (int i=0; i < nNumChords; i++)
    {
        wxLogMessage("[Cadence::GenerateFirstChord] Chord %d : %s, %s, %s, %s",
                ...
                aChords[i].GetPrintName(2).wx_str(),
                aChords[i].GetPrintName(3).wx_str() );
    }
    //END DBG ---------------------------------------------------------------
#endif
							   ...
							   Chord* pChord, int nInversion, int iPrevHChord)
{
    ...
    wxLogMessage("[Cadence::GenerateNextChord] Elegible set T: Num.notes=%d, %s, %s, %s, %s, %s",
        ...
        to_wx_string(nElegibleNote[1][2].get_english_note_name()).wx_str(),
        to_wx_string(nElegibleNote[1][3].get_english_note_name()).wx_str(),
        to_wx_string(nElegibleNote[1][4].get_english_note_name()).wx_str() );
    //END DBG ---------------------------------------------------------------
#endif
    ...
    for (int i=0; i < nNumChords; i++)
    {
        wxLogMessage("[Cadence::GenerateNextChord] Possible chord %d : %s, %s, %s, %s",
                ...
                to_wx_string(aChords[i].nNote[2].get_english_note_name()).wx_str(),
                to_wx_string(aChords[i].nNote[3].get_english_note_name()).wx_str() );
    }
    //END DBG ---------------------------------------------------------------
#endif
    ...
    wxLogMessage("[Cadence::GenerateNextChord] Selected : %s, %s, %s, %s",
        ...
        to_wx_string(aChords[iC].nNote[3].get_english_note_name()).wx_str() );
    //END DBG ---------------------------------------------------------------
#endif
...
void Cadence::Debug_DumpChord(lmHChord& oChord, int iChord)
{
    if (oChord.nReason == lm_eChordValid)
    {
		wxLogMessage("[Cadence::Debug_DumpChord] Valid chord %d : %s, %s, %s, %s",
            ...
            oChord.nNote[2].get_english_note_name().wx_str(),
            oChord.nNote[3].get_english_note_name().wx_str() );
    }
    else
    {
