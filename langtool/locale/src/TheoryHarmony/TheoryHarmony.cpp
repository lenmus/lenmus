wxString sTxt;
sTxt = _("Send your comments and suggestions to the LenMus team (www.lenmus.org)");
sTxt = _("Licensed under the terms of the GNU Free Documentation License v1.2");
sTxt = _("the teacher of music");
sTxt = _("Cover page");
sTxt = _("Theory and harmony");
sTxt = _("A collection of yet un-ordered/un-related themes on music theory.");
sTxt = _("LenMus project");
sTxt = _("Permission is granted to copy, distribute and/or modify this document under the terms of the GNU Free Documentation License, Version 1.2 or any later version published by the Free Software Foundation; with no Invariant Sections, with no Front-Cover Texts, and with no Back-Cover Texts. A copy of the license is included in the documentation received with this program, and it is also available at");
sTxt = _("http://www.gnu.org/licenses/fdl.html");
sTxt = _("Contents");
sTxt = _("Presentation");
sTxt = _("It takes a lot of time to prepare an eBbook. So I have decided to publish the chapters/themes/articles as soon as we write them, instead of waiting to have the book finished. All these provisional themes and articles are boundled into this eBook. Therefore this 'Theory and harmony' eBook is just a collection of yet un-ordered/un-related themes.");
sTxt = _("Harmony");
sTxt = _("Harmony");
sTxt = _("Cadences");
sTxt = _("In music, the term 'cadence' has several meanings. In harmony we are going to refer to a particular sequence of usually two chords whose function is to create a sensation of either stability, rest and ending, or of pause, tension and unstablility. Cadences have a function similar to that of the punctuation marks (comma, period, etc.) in written language: they mark breathing places and pauses in the music, they serve to structure the musical phrases, and they also helps to establish tonality. For example listen to the following three chords:");
sTxt = _("I");
sTxt = _("V");
sTxt = _("I");
sTxt = _("You will notice the feeling of conclusion, of reaching the end of a musical piece. The chord sequence does not create expectations of continuation. The composer could end the musical composition in the last played chord and the listener will perceive that the end has been reached and remain 'satisfied' with finishing at that point. On the contrary, listen to the following sequence of chords:");
sTxt = _("I");
sTxt = _("I");
sTxt = _("6");
sTxt = _("4");
sTxt = _("V");
sTxt = _("Now the sensation is not conclusive. It is more like a short pause and it creates expectations for continuation. If we finish the music in the last sounded chord we will leave the listener waiting for more.");
sTxt = _("Cadences are not difficult to recognize by ear but it is sometimes harder to recognize them in written music.");
sTxt = _("Cadence types");
sTxt = _("Depending on the sensation they create cadences are classified in two fundamental types:");
sTxt = _("Terminal cadences");
sTxt = _(". They create a sensation of stability, rest and ending of the musical phrase or sentence.");
sTxt = _("Transient cadences");
sTxt = _(". They are characterized for creating tension and sensation of unstablility, as well as creating expectations of continuation.");
sTxt = _("As a training exercise you will hear a cadence and based on the sensation it creates on you, try to identify if it is a terminal cadence is or transient one.");
sTxt = _("Terminal cadences");
sTxt = _("Terminal cadences create a sensation of stability and conclusion of the musical phrase or sentence. This feeling is achieved by moving to the tonic grade (grade I) from some other chord grades. Depending on the starting chord grade two types of cadences are considered:");
sTxt = _(" authentic cadences");
sTxt = _(" and");
sTxt = _(" plagal cadences");
sTxt = _(".");
sTxt = _("Authentic cadences");
sTxt = _("An authentic cadence takes place when grade V, dominant, (or V7, a V chord with an added seventh) resolves in grade I (tonic). The authentic cadence is the strongest terminal cadence, with great conclusive power.");
sTxt = _("V");
sTxt = _("I");
sTxt = _("V7");
sTxt = _("I");
sTxt = _("When the authentic cadence satisfies the following conditions:");
sTxt = _("both chords are in root position;");
sTxt = _("the first chord is the V or the V7 ; and");
sTxt = _("the melody (soprano voice) ends on the tonic pitch.");
sTxt = _("then the cadenced has its strongest conclusive power and it is the one that creates the most final sounding of all cadences. Due to this fact, if the previously listed conditions are satisfied the cadence it is named 'perfect authentic cadence' or just 'perfect cadence'. If the conditions are not satisfied the conclusive feeling of the cadence is less strong and then the cadence is named 'imperfect authentic cadence' or just 'imperfect cadence'.");
sTxt = _("There are theoretical discussions about considering the imperfect cadence as a terminal cadence. Nevertheless, we have opted to include it in the terminal group due to sonority considerations.");
sTxt = _("The perfect cadence is usually found at the end of a piece of music and the end of significant sections.");
sTxt = _("Plagal cadences");
sTxt = _("A plagal cadence takes place when grade IV (subdominant) resolves in grade I (tonic), both in root position. The plagal cadence is less strong than the authentic cadence and it is also known as the &quot;Amen&quot; cadence as it is used at the end of most church hymns.");
sTxt = _("In both cadence types, authentic and plagal, it must be taken into account that other chords playing the same harmonic functions can be used. For example, in the authentic cadence the V (dominant function) can be played by the VII. And in the plagal cadence the IV (subdominant function) can be played by the II. These replacements add variety and harmonic colour to the cadence.");
sTxt = _("IV");
sTxt = _("I");
sTxt = _("II");
sTxt = _("6");
sTxt = _("I");
sTxt = _("Exercises");
sTxt = _("Theory exercise");
sTxt = _(" - Practise to recognize on a score if a terminal cadence is authentic perfect or plagal.");
sTxt = _("Aural training");
sTxt = _(" - Train your ear and practise to identify if a terminal cadence is authentic perfect or plagal.");
sTxt = _("Terminal cadences");
sTxt = _("Authentic and plagal cadences");
sTxt = _("Theory: Identify if a terminal cadence is authentic perfect or plagal.");
sTxt = _("Terminal cadences");
sTxt = _("Authentic and plagal cadences");
sTxt = _("Aural training: Identify if a terminal cadence is authentic perfect or plagal.");
sTxt = _("Transient cadences");
sTxt = _("Transient cadences are not conclusive. They create a sensation tension and unstability and expectations for continuation. They are more like a short pause in the musical flow. There are two types of transient cadences:");
sTxt = _(" half cadences");
sTxt = _(" and");
sTxt = _(" deceptive cadences");
sTxt = _(".");
sTxt = _("Half cadences");
sTxt = _("A half cadence takes place when the harmony moves to the V degree (dominant) from any other degree, except the tonic (I). The ending on V causes it to sound incomplete or 'suspended' due to the unstableness of the dominant chord. It is considered as the weakest cadence and cannot be use to end a musical phrase.");
sTxt = _("IV");
sTxt = _("V");
sTxt = _("II");
sTxt = _("6");
sTxt = _("4");
sTxt = _("V");
sTxt = _("A particular case of the half cadence is the Phrygian half cadence. It consist of a IV chord in first inversion that resolves to a V (or V7) chord in root position. It only occurs in minor keys (the IV is minor) and was widely used during the Baroque period to end a slow musical movement in minor mode, continued by a fast movement in major mode.");
sTxt = _("IV");
sTxt = _("6");
sTxt = _("V");
sTxt = _("Another very specific case is the Andalusian cadence. It only occurs in minor keys and is given by the progression I-VII-VI-V, where the IV degree can be replaced. This cadence has a particular colour due to the chromatic half step between the root of the VII and the third of the V.");
sTxt = _("I");
sTxt = _("VII");
sTxt = _("VI");
sTxt = _("V");
sTxt = _("#");
sTxt = _("Deceptive cadences");
sTxt = _("A deceptive cadence takes place when a V chord does not resolve to a I chord, as the ear is expecting, but instead resolves to any other chord, typically to one containing the tonic pitch (the VI or the IV degrees).");
sTxt = _("V");
sTxt = _("IV");
sTxt = _("V");
sTxt = _("VI");
sTxt = _("The deceptive cadence is considered a weak cadence because of the 'surprise' and 'hanging' feel it invokes, so it is never used to end a music piece. However, it provides variety and surprise by resolving to a minor chord in major keys, and to a major chord in minor keys, and when used near the end of a piece it helps to sustain the musical interest at the moment when the final authentic cadence is expected.");
sTxt = _("Exercises");
sTxt = _("Theory exercise");
sTxt = _(" - Practise to recognize on a score if a transient cadence is a half cadence or a deceptive one.");
sTxt = _("Aural training");
sTxt = _(" - Train your ear and practise to identify if a transient cadence is a half cadence or a deceptive one.");
sTxt = _("Trasient cadences");
sTxt = _("Deceptive and half cadences");
sTxt = _("Theory: Identify if a transient cadence is a half cadence or a deceptive one.");
sTxt = _("Transient cadences");
sTxt = _("Deceptive and half cadences");
sTxt = _("Aural training: Identify if a transient cadence is a half cadence or a deceptive one.");
sTxt = _("Exercises on cadences");
sTxt = _("Once reviewed all cadence types it is important to practise to recognize and identify them. A useful trick is to associate each cadence to a melody or song you remember well. Practise to recognise cadences while you listen to music.");
sTxt = _("In this theme we just propose you some additional exercises to identify cadences, both aurally and in written music.");
sTxt = _("Exercises");
sTxt = _("Exercise 1");
sTxt = _(" - Aural training: identify the type of cadence: perfect, imperfect, plagal, half or deceptive.");
sTxt = _("Exercise 2");
sTxt = _(" - Theory: identify the type of cadence: perfect, imperfect, plagal, half or deceptive.");
sTxt = _("All cadences");
sTxt = _("All cadences");
sTxt = _("Aural training: identify the type of cadence.");
sTxt = _("All cadences");
sTxt = _("All cadences");
sTxt = _("Theory: identify the type of cadence.");
